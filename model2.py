...
def train(datasets, log_dir=None, log_description=None, save_model_dir=None):
  ...
  weights_layer3 = weight_variable([layer3['in'], layer3['out']], stddev=math.sqrt(2.0/hidden_layer2_size), name='w3')
  biases_layer3 = bias_variable([layer3['out']], name='b3')
  tf.add_to_collection('vars', weights_layer3)
  tf.add_to_collection('vars', biases_layer3)


  # Layers
  hidden1 = tf.nn.dropout(nn_layer(X_, weights_layer1, biases_layer1, 'hidden1'), keep_prob)
  
  hidden2 = tf.nn.dropout(nn_layer(hidden1, weights_layer2, biases_layer2, 'hidden2'), keep_prob)

  # Do not apply softmax yet
  output_activation = nn_layer(hidden2, weights_layer3, biases_layer3, 'output_activation', tf.identity)
  ...
  loss = loss + \
        ...
        beta * tf.nn.l2_loss(weights_layer3)
  ...
  for step in range(1, num_steps):
    ...
    if (step % (num_steps//50) == 0):
      ...
      print("Minibatch accuracy: %.5f" % acc) #accuracy.eval(feed_dict={X_: batch_data, y_: batch_labels, keep_prob: 1.0 }))
  
      if datasets.valid:
        summary_valid, acc_loss, acc_valid = session.run([merged, loss, accuracy], feed_dict={ X_: datasets.valid.data, y_: datasets.valid.labels, keep_prob: 1.0 })
        print("Validation loss: %f" % acc_loss)#accuracy.eval(feed_dict={ X_: datasets.valid.data, y_: datasets.valid.labels, keep_prob: 1.0 }))
        print("Validation accuracy: %.5f" % acc_valid)#accuracy.eval(feed_dict={ X_: datasets.valid.data, y_: datasets.valid.labels, keep_prob: 1.0 }))
        if log_dir:
  ...
  if datasets.valid:
    ...
    print("Final validation accuracy: %.5f" % acc_valid)#accuracy.eval(feed_dict={ X_: datasets.valid.data, y_: datasets.valid.labels, keep_prob: 1.0 }))
    if log_dir:
  ...
  if datasets.test: 
    ...
    print("Test loss: %.5f" % loss_test)#accuracy.eval(feed_dict={ X_: datasets.test.data, y_: datasets.test.labels, keep_prob: 1.0 }))
    print("Test accuracy: %.5f" % acc_test)#accuracy.eval(feed_dict={ X_: datasets.test.data, y_: datasets.test.labels, keep_prob: 1.0 }))
...
if __name__ == "__main__":
  ...
  # Seasonal was best in 1hidden layer model
